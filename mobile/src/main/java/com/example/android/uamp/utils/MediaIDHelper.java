...
public class MediaIDHelper {
     ...
     *将媒体可浏览类别（如果有）和唯一音乐ID（如果有）编码到单个字符串媒体ID中。
     ...
     *                   表示此项目的浏览父项的类别层次结构
     * @return a hierarchy-aware media ID  层次感知媒体ID
     *
     * 返回一个多类别的MediaID,由/或|分割
     */
        ...
        if (categories != null) {
            for (int i=0; i < categories.length; i++) {
                ...
                if (i < categories.length - 1) {
                    sb.append(CATEGORY_SEPARATOR);//  拼接/
                }
            }
        }
        if (musicID != null) {
     ...
     * exists in more than one music list, and thus we are able to correctly build the playing queue.
     *
     * @param mediaID that contains a category and categoryValue.包含类别和类别值。
     *                获取层次数组
     */
